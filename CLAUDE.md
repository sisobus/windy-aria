...
├── src/
│   ├── interpreter/
│   │   └── windy.ts       # Session API step-by-step → InstructionEvent + IpState/TickFrame
│   ├── audio/
│   │   ├── synth.ts       # Web Audio 합성기 (osc + ADSR + noise + gliss)
│   │   ├── mapping.ts     # Opcode → SoundEvent
│   │   └── engine.ts      # 시퀀스 스케줄러 (BPM 기반)
│   ├── visualizer/
│   │   ├── Grid.tsx       # 2D IP 커서 grid (audio clock 동기화)
│   │   └── colors.ts      # IP 색상 팔레트 (birth-order index)
│   ├── songs/
│   │   ├── index.ts       # frontmatter loader (`---` separator)
│   │   └── *.wnd          # 큐레이션된 곡 (title/intent/bpm + 소스)
│   ├── share/
│   │   └── url.ts         # `#s=base64url(utf8)&bpm=N` permalink (windy 호환)
│   ├── export/
│   │   └── wav.ts         # OfflineAudioContext + RIFF WAV 인코더 + downloadBlob
│   ├── types.ts           # 35 opcode 타입 + 이벤트 타입
│   ├── App.tsx            # 메인 UI (코드 에디터 + Play + Visualizer + Gallery + Share + WAV)
│   └── main.tsx
├── docs/
│   └── MAPPING.md         # 풍향 → 사운드 매핑 v1 명세
├── scripts/
│   └── verify-songs.mjs   # 모든 src/songs/*.wnd 가 cap 안에 halt 하는지 점검
├── index.html
├── package.json
├── tsconfig*.json
└── vite.config.ts
...
- ✅ **2D 커서 시각화** — `src/visualizer/Grid.tsx` 가 모든 live IP 를 grid 위에 색깔별로 그림. Play 모드는 audio clock 기준 rAF 로 동기화, Debug 모드는 step 마다 snapshot.ips 갱신, 충돌 셀은 그라디언트 stripe.
- ✅ **큐레이션된 곡 갤러리 (인프라 + 6곡 시드)** — `src/songs/*.wnd` (frontmatter `title`/`intent`/`bpm` + `---` 분리자). App 에 Gallery panel — 클릭 한 번에 source/BPM 로드 + 재생. `scripts/verify-songs.mjs` 가 cap 안에 halt 하는지 점검. 6곡 시드: `stair`, `bounce`, `dance`, `river`, `gust`, `storm-echo`. 메인테이너의 자체 작곡으로 ~30곡까지 확장 예정.
- ✅ **공유 URL** — `src/share/url.ts` 가 `#s=<base64url(utf8(source))>&bpm=N` 인코딩. 압축 없이 plain base64url 으로 windy 본 페이지 (`web/main.js`) 의 `#s=...` 컨벤션과 호환. 페이지 로드 시 hash 디코드 → 에디터 자동 채움, 코드/BPM 변경 시 300ms debounce 로 history.replaceState. 툴바에 "copy share link" 버튼 + 토스트.
- ✅ **WAV export** — `src/export/wav.ts` 가 `OfflineAudioContext` 로 동일한 Synth 를 돌려 16-bit mono PCM WAV 로 인코딩 (44바이트 RIFF 헤더 직접 작성, 외부 의존성 0). Synth/Engine 의 `ctx` 타입은 `BaseAudioContext` 로 widen 해서 live/offline 양쪽 다 같은 합성 경로를 탄다. App 의 Play 영역에 "Download WAV" 버튼 — 파일명 `windy-aria-${bpm}bpm-${ts}.wav`.
...
| 영역 | 위치 / 비고 |
|---|---|
| 갤러리 곡 수 확장 (~30곡) | `src/songs/` 에 시드 6곡 — 메인테이너가 직접 작곡으로 채워나가는 영역. v1 코드 작업으로는 완결, 콘텐츠만 남음 |
