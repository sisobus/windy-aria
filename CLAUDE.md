...
│   ├── App.tsx            # 메인 UI (코드 에디터 + Play + Visualizer + Gallery + Share)
│   └── main.tsx
├── docs/
...
| 갤러리 곡 수 확장 (~30곡) | `src/songs/` 에 시드 6곡 — 메인테이너가 직접 작곡으로 채워나가는 영역 |
...
각 항목은 **이 windy-aria 디렉토리에서 새 Claude Code 세션을 열고** 그대로 붙여넣으면 됩니다. 권장 진행 순서: 1 → 2 → 3.
...
### 3. 다중 IP polyphony (cross-repo, v1.1)
