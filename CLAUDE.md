...
│   ├── App.tsx            # 메인 UI (코드 에디터 + Play + Visualizer)
│   └── main.tsx
├── docs/
...
향후 추가 예정: `src/export/` (WAV/공유 URL), `public/songs/` (갤러리), `tests/`.
...
각 항목은 **이 windy-aria 디렉토리에서 새 Claude Code 세션을 열고** 그대로 붙여넣으면 됩니다. 권장 진행 순서: 1 → 2 → 3 → 4 → 5.

### 1. 큐레이션된 곡 갤러리 (사용자 본인이 첫 작곡가)
...
### 5. 다중 IP polyphony (cross-repo, v1.1)
